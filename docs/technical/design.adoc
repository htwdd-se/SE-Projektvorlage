...
Architekturdokument (`architecture.adoc`) beschrieben. Dieses Dokument vertieft die interne Sicht._

_Das Datenmodell hier beschreibt die technische Struktur der Persistenz. Die fachliche Bedeutung der Entitäten ist im Domänenmodell (`domain_glossary.adoc`) definiert._
...
package "Frontend" {
  component "[z. B. Buchungskomponente]" as booking_ui
  component "[z. B. Authentifizierungskomponente]" as auth_ui
  component "[z. B. Gemeinsame UI-Komponenten]" as shared_ui
}

package "Backend" {
  component "[z. B. Buchungs-Controller]" as booking_ctrl
  component "[z. B. Buchungs-Service]" as booking_svc
  component "[z. B. Nutzer-Service]" as user_svc
  component "[z. B. Benachrichtigungs-Service]" as notification_svc
}

package "Persistenz" {
  component "[z. B. Buchungs-Repository]" as booking_repo
  component "[z. B. Nutzer-Repository]" as user_repo
  database "Datenbank" as db
}

rectangle "Externer Dienst\n[z. B. E-Mail]" as ext

booking_ui --> booking_ctrl : REST
auth_ui --> user_svc : REST
booking_ctrl --> booking_svc
booking_svc --> booking_repo
booking_svc --> notification_svc
user_svc --> user_repo
booking_repo --> db
user_repo --> db
notification_svc --> ext
...
_Sequenzdiagramme zeigen, wie Komponenten bei typischen Abläufen zusammenarbeiten.
Dokumentiert werden hier nur die wichtigsten und komplexesten Flüsse —
nicht jede einfache CRUD-Operation._
